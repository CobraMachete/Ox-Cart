...
(function (ftrack, ftrackWidget) {
	...
	async function getServerLocationId() {
		...
		// Name is usually exactly "ftrack.server"
	...
	/**
	* Ask the ftrack server for a fresh, proxied download URL for a component.
	* We try a couple of known endpoints; different server versions expose different paths.
	*/
	async function resolveComponentUrlViaServerProxy(componentId) {
		...
		// Try a few likely endpoints. session.call accepts relative paths.
		const candidates = [
			// common on newer installs:
			`/api/locations/${locId}/components/${componentId}/download-url`,
			// sometimes:
			`/api/locations/${locId}/components/${componentId}/signed-url`,
			// generic download proxy (if available):
		...
		for (const path of candidates) {
			try {
				...
				// Most endpoints reply with { url: "https://..." } or { data: { url: "..." } }
	...
	async function loadSeedDataFromHashAndApi() {
		...
		// Prefer direct URLs from hash (may work if CORS allows and not expired)
		let teams     = await tryFetchJson(hp.teams_url);
		let structure = await tryFetchJson(hp.structure_url);
		
		// If that failed, first try the **server-proxy** URL for each component
		...
		// If that still failed, fall back to the raw component location URL (may 403)
		if (!teams && hp.teams_comp) {

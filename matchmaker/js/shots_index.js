...
(function (ftrack, ftrackWidget) {
	...
	async function fetchSeedFromHash() {
		...
		return { teams, structure, entityId: p.entity_id || null, projectId: p.project_id || null };
	}
	...
	async function onWidgetLoad(payload) {
		...
		if (!session) {
			...
			try {
				...
				fetchSeedFromHash()
				.then(({teams, structure}) => {
					if (!teams || !structure) {
						console.warn('Seed URLs present but fetch failed or missing data.', {teams: !!teams, structure: !!structure});
					}
					// hydrate your UI…
				})
				.catch(err => console.error('Error fetching seed from hash:', err));
			} catch (err) {

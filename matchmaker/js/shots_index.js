...
(function (ftrack, ftrackWidget) {
	...
	async function bootSeedViaEventHub(session, entity) {
		const correlationId = (crypto.randomUUID?.() || (Math.random().toString(36).slice(2) + Date.now()));
		
		// 1) Listen for the seed once.
		await session.event_hub.subscribe('topic=tntsports.matchmaker.seed', function (event) {
			const d = event && event.data || {};
			if (d.correlation_id !== correlationId) return;             // not ours
			if (d.entity_id !== entity.id) return;
			
			console.debug('[MatchMaker] seed received:', d);
			
			// Merge into your boot state and kick your UI
			const partial = {
			...
			window.MATCHMAKER_BOOT = Object.assign({}, window.MATCHMAKER_BOOT || {}, partial);
		...
		// 2) Ask the server for the seed
		session.event_hub.publish({
			topic: 'tntsports.matchmaker.request',
			data: {
				correlation_id: correlationId,
				entity_id: entity.id,
				// Optional; the server can derive it if omitted:
